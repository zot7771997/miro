...
void Chassis::Init()
{
	...
	/*
	Serial.println("КАЛИБРОВОЧНАЯ ТАБЛИЦА ДВИГАТЕЛЕЙ");
	for (byte w = 0; w < WHEEL_COUNT; w++)
	{
		Serial.print("Колесо ");
		Serial.println(w);
...
void Chassis::Sync()
{
	...
	}//Расчитали угловые скорости колес

	/*
	if (this->_wheel_sync_move)//Если задано сихронное вращение колес
	{
		byte w;
	...
	}//Если была задача на синхронное движение и хотябы одно из колес его выполнило - останов остальных колес
	*/
	...
	for (byte w = 0; w < WHEEL_COUNT; w++)
	{
		...
		{
			if (_syncloop[w]) //Колесо вращается и произошло очередное пересечение маркерной линейки колеса
			{
				_syncloop[w] = false;
...
int  Chassis::wheelRotateAng(float speed, float ang, byte wheel)
{
	...
	dist_wheelTachomCount[wheel] = _wheelTachomCount[wheel] + int(0.51 + (fabs(this->_wheelSetAng[wheel]) / float(SEGMENT_ANGLE))); //число сегментов маркерной линейки колеса, которое нужно отсчитать, чтобы повернуть на требуемый угол;
	this->_vbat = getVoltage();
